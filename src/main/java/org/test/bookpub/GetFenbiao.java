...
public class GetFenbiao {
    public static void main(String[] args) {

        int itemSize = 7;
        File file = new File("D:\\gaozhong20170728.txt");
        try {
...
//                temp.add(t[0].replace("：", "").replace("，", "").replace(",", ""));
                ...
                if (t[1].startsWith("7月") || t[1].startsWith("8月")) {
                    ...
                    temp.add(t[1].substring(9));
                   /* String time[] = t[1].substring(9).split("-");
                    temp.add(time[0]);
                    if (time.length == 2)
                        temp.add(time[1]);
                    else
            ...
            while (items.get(index).size() != itemSize) {
                count++;
                index--;
            }
...
//                items.get(index++).add(String.valueOf(suoyin));
            ...
            writeExcel("D:\\gaozhongfenbiao2017-0728.xls", items);

/*            WritableWorkbook book = null;
            try{
                // Excel获得文件
                Workbook wb = Workbook.getWorkbook(new File("D:\\abcd.xls"));
                // 打开一个文件的副本，并且指定数据写回到原文件
                book = Workbook.createWorkbook(new File("D:\\abcd.xls"), wb);

                // 添加一个工作表
                WritableSheet sheet = book.getSheet("sheet1");

                int c = 1;
                for (List<String> t : items) {
                    int d = 1;
                    for (String p : t) {
                        sheet.addCell(new Label(d++, c, p));
                    }
                    c++;
                }
