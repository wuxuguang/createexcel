package org.test.bookpub.gaozhong;
...
public class GaoZhongGetExcelInfo {
    public static void main(String[] args) {
        String timeDuration = "20171002-20171008";
        File file = new File("E:\\瞄之政治\\资料ALL\\20170729备份\\教研\\20171002-20171008\\gaozhong.txt");
        try {
                        ...
                        if (a.startsWith("第一次") || a.startsWith("第1次")) {
                            ...
                            while (n >= 0 && n < j) {
                                if (!isEnter){
                                    ...
                                    xueke = sub.substring(1).split(" ")[0].split(":")[1];
                                    huibaoren =  sub.substring(1).split(" ")[1].split(":")[1];
                                    q++;
                                }
                                items.get(n).add(timeDuration);
                                items.get(n).add(xueke);
                                items.get(n).add(huibaoren);
                                items.get(n).add("(" + m + "/" + count + ")");
//                                items.get(n).add("第" + m + "次");
                                n++;
                                m++;
                            }
                        }
...
//                    temp.add(t[1].substring(t[1].indexOf("日") + 1));
                    ...
                    if (t[1].length() >9){
                        ...
                        temp.add(t[1].substring(9).split("-")[0]);
                        temp.add(t[1].substring(9).split("-")[1]);
                    }else {
                        temp.add("——");
                        temp.add("——");
                        temp.add("——");
            ...
            while (index > -1 && items.get(index).size() != 10) {
                count++;
                index--;
            }
            ...
            while (index < items.size()) {
                String sub = subjects.get(q);
                items.get(index).add(timeDuration);
                items.get(index).add(sub.substring(1).split(" ")[0].split(":")[1]);
                items.get(index).add(sub.substring(1).split(" ")[1].split(":")[1]);
                items.get(index++).add("(" + suoyin + "/" + count + ")");
//                items.get(index++).add("第" + suoyin + "次");
                suoyin++;
            }

            writeExcel("E:\\瞄之政治\\资料ALL\\20170729备份\\教研\\20171002-20171008\\gaozhong.xls", items);
